...
class Genapp_Service_GenericService {
	...
	public function dataToGridDetailJSON($id, $data) {
		...
		if (!empty($data)) {
			...
			return '{'.'success:true'.', id:'.json_encode($id).', title:'.json_encode($firstData->tableFormat->label.' ('.count($locationsData).')').', hasChild:'.json_encode($hasChild).', columns:'.json_encode($columns).', fields:'.json_encode($locationFields).', data:'.json_encode($locationsData).'}';
		} else {
