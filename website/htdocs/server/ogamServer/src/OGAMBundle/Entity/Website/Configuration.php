...
class Configuration {
	 ...
	 * Add a new parameter.
	 *
	 * @param ArrayApplicationParameter] $params
	 */
	...
	function getConfig($name, $defaultValue = null, $silent = false) {
		...
		if (!empty($parameter)) {
			...
			if (!$silent) {
				...
				$logger->warning('Configuration parameter ' . $name . ' not found, using default value : ' . $defaultValue);
			}
			...
			// Missing config
			$logger = $this->get('logger');
			$logger->error('Configuration parameter ' . $name . ' cannot be found');
