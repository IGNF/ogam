<?php
namespace OGAMBundle\Test\Helper;

use OGAMBundle\Helper\GenerateEditLink;
...
class GenerateEditLinkTest extends \PHPUnit_Framework_TestCase
{
    public function testGetEditLink(){
        ...
        $field1->setValue('1')->setMetadata((new TableField())->setData(new Data('PROVIDER_ID'))->setFormat($format), null);
        $data->addIdField($field1);
        $field2= new GenericField('PLOT_DATA', 'PLOT_CODE');
        $field2->setValue('01575-14060-4-0T')->setMetadata((new TableField())->setData(new Data('PLOT_CODE'))->setFormat($format), null);
        $data->addIdField($field2);
        $field3 = new GenericField('PLOT_DATA', 'CYCLE');
        $field3->setValue('5')->setMetadata((new TableField())->setData(new Data('CYCLE'))->setFormat($format), null);
