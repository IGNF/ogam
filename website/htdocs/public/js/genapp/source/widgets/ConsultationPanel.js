...
Genapp.ConsultationPanel = Ext
		.extend(
				Ext.Panel,
				{
					...
					/**
					 * @cfg {Boolean} hideGridCsvExportMenuItem if true hide the
					 *      grid csv export menu item (defaults to false).
					 */
					hideGridCsvExportMenuItem : false,
					/**
					 * @cfg {Boolean} hideAggregationCsvExportMenuItem if true
					 *      hide the aggregation csv export menu item (defaults
					 *      to false).
					 */
					hideAggregationCsvExportMenuItem : false,
					/**
					 * @cfg {Boolean} hideInterpolationButton if true hide the
					 *      interpolation button (defaults to false).
					 */
					hideInterpolationButton : false,
					/**
					 * @cfg {Boolean} hideAggregationButton if true hide the
					 *      aggregation button (defaults to false).
					 */
					 ...
					 *      button in map toolbar (defaults to false).
					 */
					hideMapDetails : true,
					/**
					 ...
					 */
					userManualLinkHref : Genapp.base_url
							+ 'pdf/User_Manual.pdf',
					/**
					...
					/**
					 * @cfg {String} csvExportButtonText The csv Export Button
					 *      Text (defaults to <tt>'Export CSV'</tt>)
					 */
					csvExportButtonText : 'Csv Export',
					/**
					 * @cfg {String} gridCsvExportMenuItemText The grid Csv
					 *      Export Menu Item Text (defaults to
					 *      <tt>'Results'</tt>)
					 */
					gridCsvExportMenuItemText : 'Results',
					/**
					 * @cfg {String} aggregationCsvExportMenuItemText The
					 *      aggregation Csv Export Menu Item Text (defaults to
					 *      <tt>'Aggregation cells'</tt>)
					 */
					aggregationCsvExportMenuItemText : 'Aggregation cells',
					/**
					 * @cfg {String} interpolationButtonText The interpolation
					 *      Button Text (defaults to <tt>'Interpolation'</tt>)
					 */
					interpolationButtonText : 'Interpolation',
					/**
					 * @cfg {String} aggregationButtonText The aggregation
					 *      Button Text (defaults to <tt>'Aggregation'</tt>)
					 */
        ...
        <li>Uncheck 'Browse in same window'</li> \
					...
					/**
					 * @cfg {Ext.SplitButton} aggregationButton The aggregation
					 *      button
					 */
					/**
					 * @cfg {Ext.SplitButton} interpolationButton The
					 ...
					 */
					/**
					 * @cfg {Ext.menu.Item} gridCsvExportMenuItem The grid csv
					 *      export menu item
					 */
					/**
					 * @cfg {Ext.menu.Item} aggregationCsvExportMenuItem The
					 *      aggregation csv export menu item
					 */
					/**
					 * @cfg {Ext.menu.Menu} aggregationButtonMenu The
					 *      aggregation button menu
					 */
					/**
					 ...
					 */
					/**
					 * @cfg {Ext.menu.Menu} interpolationButtonMenu The
					 *      interpolation button menu
					 */
					/**
					 ...
					 *      interpolationButtonMenuGridsCombo The interpolation
					 *      button menu grids combo
					 */
					/**
					 * @cfg {Ext.form.ComboBox}
					 *      interpolationButtonMenuMethodsCombo The
					 *      interpolation button menu methods combo
					 */
					/**
					 * @cfg {Ext.form.ComboBox}
					 *      interpolationButtonMenuMaxDistanceText The
					 *      interpolation button menu max distance text
					 */
					/**
					 * @cfg {Ext.form.ComboBox}
					 *      interpolationButtonMenuMaxDistanceTextDefaultValue
					 *      The interpolation button menu max distance text
					 *      default value (default to 5000)
					 */
					...
					initComponent : function() {
											...
											fn : function(store, records,
												...
												for (i = 0; i < records.length; i++) {
													if (records[i].data.is_default === '1') {
														...
														this
																.updateDatasetFormsPanel(records[i].data.id);
														break;
													}
												}
											},
									...
									listeners : {
										'select' : {
											fn : function(combo, record, index) {
												this
														.updateDatasetFormsPanel(record.data.id);
											},
											scope : this
										}
									}
						...
						this.centerPanel
								.on(
										'render',
										function(tabPanel) {
											var tabEdgeDiv = tabPanel.getEl()
													.query(".x-tab-edge");
											if (!this.hideUserManualLink) {
												var userManualLinkEl = Ext.DomHelper
														.insertBefore(
																tabEdgeDiv[0],
																{
																	tag : 'li',
												...
												// Stop the event propagation to
												// avoid the TabPanel error
												userManualLinkEl
														.on(
																'mousedown',
																Ext.emptyFn,
											...
											function addTopButton(config) {
												...
												// Set the ul dom to the size of
												// the TabPanel instead of
												// 5000px by default
												...
												return new Ext.ComponentMgr.create(
														Ext.apply({
															renderTo : el.id
														}, config));
											}
																...
																items : [
																		...
																		this.interpolationButtonMenuDataCombo = new Ext.form.ComboBox(
																				{
																					...
																					store : new Ext.data.JsonStore(
																							{
																								url : Genapp.base_url
																										+ 'interpolation/ajaxgetvariables',
																								method : 'POST',
																								listeners : {
																									load : function(
																											store,
																											...
																											options) {
																										if (records.length == 0) {
																											this.interpolationButtonMenuDataCombo
																													.reset();
																				...
																				}),
																		{
																			...
																			handler : function(
																					b,
																					e) {
																				if (this.interpolationButtonMenuDataCombo
																						.isValid(true)
																						&& this.interpolationButtonMenuGridsCombo
																								.isValid(true)
																						&& this.interpolationButtonMenuMethodsCombo
																								.isValid(true)
																						&& this.interpolationButtonMenuMaxDistanceText
																								.isValid(true)) {
																					this
																							.showMask();
																					Ext.Ajax
																							.request({
																								url : Genapp.base_url
																										+ 'interpolation/ajax-validate-interpolation-variable-form',
																								success : function(
																										response,
																										options) {
																									var response = Ext
																								...
																								failure : function() {
																									...
																									Ext.Msg.alert
																											.createCallback(
																													'Error...',
																													'An error occured during the interpolation process.');
																								},
																								params : {
																									'INTERPOLATION_VARIABLE' : this.interpolationButtonMenuDataCombo
																											.getValue(),
																									'GRID_NAME' : this.interpolationButtonMenuGridsCombo
																											.getValue(),
																									'METHOD' : this.interpolationButtonMenuMethodsCombo
																											.getValue(),
																									'MAXDIST' : this.interpolationButtonMenuMaxDistanceText
																											.getValue()
																								},
																								scope : this
																							});
																					this.interpolationButtonMenu
																							.hide();
																				}
																			},
																			scope : this,
																...
																items : [
																					...
																					store : new Ext.data.JsonStore(
																							{
																								...
																								listeners : {
																									load : function(
																											...
																											options) {
																										if (records.length == 0) {
																											this.aggregationButtonMenuDataCombo
																													.reset();
																				...
																				}),
																		{
																			...
																			handler : function(
																					b,
																					e) {
																				if (this.aggregationButtonMenuDataCombo
																						.isValid(true)
																						&& this.aggregationButtonMenuGridsCombo
																								.isValid(true)) {
																					this
																							.showMask();
																					Ext.Ajax
																							.request({
																								url : Genapp.base_url
																										+ 'aggregation/ajax-validate-aggregation-variable-form',
																								success : function(
																										response,
																										options) {
																									var response = Ext
																									...
																									if (Ext
																										...
																										this
																												.getStatus(
																														'aggregation',
																														function() {
																															this.aggregationCsvExportMenuItem
																																	.enable();
																															this.mapPanel
																																	.disableLayersAndLegends(
																																			this.mapPanel.layersActivation['aggregation'],
																																			true,
																																			true,
																																			true);
																															this.mapPanel
																								...
																								failure : function() {
																									...
																									Ext.Msg.alert
																											.createCallback(
																													'Error...',
																													'An error occured during the aggregation process.');
																								},
																								params : {
																									'AGGREGATE_VARIABLE' : this.aggregationButtonMenuDataCombo
																											.getValue(),
																									'GRID_NAME' : this.aggregationButtonMenuGridsCombo
																											.getValue()
																								},
																								scope : this
																							});
																					this.aggregationButtonMenu
																							.hide();
																				}
																			},
																			scope : this,
											...
											if (!this.hideGridCsvExportMenuItem) {
												csvExportMenuItems
														.push(this.gridCsvExportMenuItem = new Ext.menu.Item(
																{
											...
											if (!this.hideAggregationCsvExportMenuItem) {
												csvExportMenuItems
														.push(this.aggregationCsvExportMenuItem = new Ext.menu.Item(
																{
						...
						if (!this.hideRequestVerticalLabel) {
							this
									.addVerticalLabel(this.queryPanel,
											'genapp-query-request-panel-ct-xcollapsed-vertical-label-div');
						}
						...
						if (!this.hideDetailsVerticalLabel) {
							this
									.addVerticalLabel(this.detailsPanelCt,
											'genapp-query-details-panel-ct-xcollapsed-vertical-label-div');
						}
						...
						Genapp.ConsultationPanel.superclass.initComponent
								.call(this);
					},
					...
					updateWestPanels : function(response, opts, apiParams,
						...
						for ( var i = 0; i < forms.data.length; i++) {
							if (!(Ext.isEmpty(forms.data[i].criteria) && Ext
									.isEmpty(forms.data[i].columns))) {
					...
					resetRequest : function() {
						this.updateDatasetFormsPanel(this.datasetComboBox
								.getValue());
					},
					...
					submitRequest : function() {
						// Disable the top buttons, reset the combos and force a
						// reload
						if (!this.hideAggregationButton) {
						...
						if (!this.hideInterpolationButton) {
							...
							this.interpolationButtonMenuMaxDistanceText
									.setValue(this.interpolationButtonMenuMaxDistanceTextDefaultValue);
							delete this.interpolationButtonMenuDataCombo.lastQuery;
						}
						if (!this.hideCsvExportButton) {
						...
						this.mapPanel.disableLayersAndLegends(
								this.mapPanel.layersActivation['request'],
								true, false, true);
						this.mapPanel.disableLayersAndLegends(
								this.mapPanel.layersActivation['aggregation'],
								true, true, true);
						this.mapPanel
								.disableLayersAndLegends(
										this.mapPanel.layersActivation['interpolation'],
										true, true, true);
						...
						if (!this.mapResultLayers) {
							...
							for ( var i = 0; i < rla.length; i++) {
								var layer = this.mapPanel.map
										.getLayersByName(rla[i])[0];
								// The layer visibility must be set to true to
								// handle the 'loadend' event
								layer.events
										.register(
												"loadend",
												this,
						...
						if (!this.mapMask) {
							this.mapMask = new Ext.LoadMask(this.mapPanel
									.getEl(), {
								msg : this.mapMaskMsg
							});
						}
								...
								.submit(
										{
											...
											success : function(form, action) {
												...
												// Creation of the column model
												// and the reader metadata
												// fields
												var columns = action.result.columns;
												var newCM = new Array(
														{
															header : '',
															renderer : this.renderLeftTools
																	.createDelegate(this),
															sortable : false,
															fixed : true,
															menuDisabled : true,
												...
												for ( var i = 0; i < columns.length; i++) {
													columnConf = {
														header : Genapp.util
																.htmlStringFormat(columns[i].label),
														sortable : true,
														dataIndex : columns[i].name,
														width : 100,
														tooltip : Genapp.util
																.htmlStringFormat(columns[i].definition),
													...
													newRF
															.push(readerFieldsConf);
												}

												// Updates of the store reader
												// metadata
												this.gridDSReader
														.updateMetadata({
															root : 'rows',
												...
												// The grid panel must be
												// rendered and activated to
												// resize correctly
												// the grid's view in proportion
												// of the columns number
												if (this.centerPanel
														.getActiveTab() instanceof Genapp.MapPanel) {
													this.centerPanel
															.activate(this.gridPanel);
					...
					updateFormsPanel : function(requestParams, apiParams,
						...
						Ext.Ajax.request({
							...
							success : this.updateWestPanels.createDelegate(
									this, [ apiParams, criteriaValues ], true),
							method : 'POST',
					...
					exportCSV : function(actionName) {
						var launchCsvExport = function(buttonId, text, opt) {
							this.showMask(true);
							window.location = Genapp.ajax_query_url
									+ actionName;
						};
					...
					printMap : function(button, event) {
						...
						// Get the layers
						var activatedLayers = this.mapPanel.map.getLayersBy(
								'visibility', true);
						...
						activatedLayersNames = activatedLayersNames.substr(0,
								activatedLayersNames.length - 1);

						Genapp.util.post(Genapp.base_url
								+ 'map/ajaxgeneratemap', {
							center : center,
							zoom : zoom,
					 ...
					 *            by default)
					 */
					numberPattern : function(decimalSeparator,
							decimalPrecision, groupingSymbol) {
					...
					getStatus : function(serviceName, callback) {
						Ext.Ajax
								.request({
									url : Genapp.base_url + serviceName
											+ '/ajax-get-status',
									success : function(response, options) {
										var response = Ext
												.decode(response.responseText);
										if (Ext.isEmpty(response.success)
												|| response.success == false) {
											this.hideMask();
											...
											if (response.status == 'RUNNING') {
												this.getStatus.defer(2000,
														this, [ serviceName,
																callback ]);
											} else if (response.status == 'OK') {
												this.hideMask();
												callback.call(this);
											} else { // The service is done
												// or an error occured
												this.hideMask();
												var msg = 'An error occured during the status request.';
												if (!Ext
						...
						Ext.Ajax
								.request({
											...
											if (Ext.isEmpty(response.success)
												...
												if (this.autoZoomOnResultsFeatures == true) {
													if (this.mapPanel.resultsBBox !== null) {
														this.mapPanel
																.zoomOnBBox(this.mapPanel.resultsBBox);
													}
									...
									failure : function(response, options) {
										var msg = 'An error occured during the bounding box request. Status code : '
												+ response.status;
										Ext.Msg.alert('Error...', msg);
									},
									scope : this
