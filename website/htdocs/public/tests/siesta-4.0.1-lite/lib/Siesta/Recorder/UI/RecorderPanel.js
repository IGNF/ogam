...
Ext.define('Siesta.Recorder.UI.RecorderPanel', {
    ...
    onRecordClick : function () {
        ...
        if (descriptor) {
            ...
            if (this.isRecording()) {
                ...
                if (!test.scopeProvider || pageUrl && test.scopeProvider.sourceURL !== pageUrl) {
    ...
    generateSteps : function (events) {
        ...
        Ext.Array.forEach(events || this.store.getRange(), function (ev) {
            var step = ev.isLeaf() ? ev.asStep(t) : me.generateSteps(ev.childNodes);
    ...
    onActionAdded : function (event, action) {
        ...
        targetParent.appendChild(actionModel);
    ...
    onBeforeEdit : function (cellEditing, editingContext) {
        ...
        if (editingContext.column.dataIndex === 'target') {
    ...
    onPlaySingleStepClick : function (cmp, row) {

        if (this.test) {
            var action = this.store.getAt(row);
    ...
    onPlayFromStepClick : function (cmp, row) {
        if (this.test) {
            this.fireEvent('play', this, this.test);
            this.test.chain(this.generateSteps(this.store.getRange(row)));
        }
    },
